...
public class D_MatrixArray {
	/*
	 * * 2차원 배열 : 배열 안에 배열.
	 * 			"같은" 자료형으로 구성 된 "1차원 배열의 묶음"
	 * 
	 *  * 2차원 배열은 할당 된 공간마다 인덱스 번호를 "2 개" 부여
...
//		method1();
	...
	public static void method1() {
		...
		// 크기가 4인 1차원 배열 3개 묶음으로 할당 (3행 4열)
//		arr = new int[3][4];
		arr = new int[3][];
		...
		*/
		...
		for (int raw = 0; raw < arr.length; raw++) {
			...
			for(int i = 0; i < arr[raw].length; i++) {
				// i : 열 인덱스, 0 ~ (열 크기 - 1)
				System.out.print( arr[raw][i] + " " );
			}
		...
		for (int raw = 0; raw < arr2.length; raw++) {
			// raw : 행 인덱스, 0 ~ (행 크기 - 1)
			
			for(int i =0; i < arr2[raw].length; i++) {
				// i : 열 인덱스, 0 ~ (열 크기 - 1)
				System.out.print( arr2[raw][i] + " " );
			}
