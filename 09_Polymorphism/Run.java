...
public class Run {
	...
	public static void main(String[] args) {
		...
		//		물려 받은 멤버(부모 클래스의 정의된 멤버),
		//		내가 정의한 멤버(자식 클래스의 멤버) 모두 접근 가능
		...
		// 가능한 이유? 상속 관계이기 때문에 
		// [1] 자바에서 상속 관계는 'IS-A 관계'를 성립 시킨다.
		// [2] 자식 인스턴스(객체)는 heap 메모리에 생성 시 부모의 멤버 공간을 항상 내포하고 있다.
		// [3] 따라서 컴파일러는 부모 타입의 참조변수로 자식 인스턴스를 가리키는 것을 '타입 안전'하다고 판단한다.
		// [4] 이 변환은 형변환 연산자를 생략할 수 있는 '자동(묵시적) 형변환'으로 처리된다.
		...
		// pm1.getCardNumber(); // => 컴파일 오류 발생! 실제 힙 메모리에는 생성되지만, 
		//							  참조 변수(pm1)는 알 수 없으므로 오류 발생!!
		...
		for (int i = 0; i < arr.length; i++) {
			...
			// processPay() : 각 자식 타입에서 재정의 된 메소드 내용이 실행 (동적 바인딩)
		}
		
		// * 다운 캐스팅 : 업 캐스팅으로 인해 제한된 자식 고유의 기능을 사용하기 위해,
		//				자식 타입으로 강제로 형변환 하는 것.
		// => 자식 타입으로의 변환은 heap 메모리에 해당 영역이 존재하는지 컴파일러가 확신할 수 없으므로,
		//	  반드시 명시적(강제) 형변환을 통해 처리해야 한다.
		//		-> 형변환 연산자 : (자식 클래스명)참조변수
		...
		// 포인트 결제 방식 => 포인트 점수 출력
		for (int i = 0; i < arr.length; i++)	{
			// 참조 변수 instaceof 확인할타입(자식클래스명) : boolean
