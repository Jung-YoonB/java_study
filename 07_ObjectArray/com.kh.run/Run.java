...
public class Run {
	...
	public static void main(String[] args) {
		...
		sArr[0] = new Student("배정윤", 20, 80.5);	// sArr -> [ 배정윤 학생의 정보, null, null ]
		sArr[1] = new Student("최재욱", 20, 95.2);	// sArr -> [ 배정윤 학생의 정보, 최재욱 학생의 정보, null ]
		sArr[2] = new Student("조빌립", 20, 100.0);	// sArr -> [ 배정윤 학생의 정보, 최재욱 학생의 정보, 조빌립 학생의 정보 ]
		...
		*/
		...
		//	2) 학생정보 출력 기능(메소드)가 있는지? 없으면 getter 메소드 활용! => printInfo()
		sArr[1].printInfo();
		
		// sArr.printInfo(); // => 오류 발생! 배열(sArr)은 printInfo 라는 기능을 가지고 있지 않음!
		
		// * 세번째 학생 정보 삭제
		sArr[2] = null;	// sArr -> [ 배정윤 학생의 정보, 최재욱 학생의 정보, null ]
		
		// sArr[2].printInfo(); // => 객체가 제거되었는데 기능을 호출하려고 함!
...
//			sArr2[i] = new Student(sc.next(), sc.nextInt(), sc.nextDouble());
		}
		
		// * 출력
		System.out.println("=== 학생 정보 ===");	
		// for (자료형 변수명 : 배열명) {}
		for (Student s : sArr2) {	// s -> sArr[i]
			s.printInfo();
		}
